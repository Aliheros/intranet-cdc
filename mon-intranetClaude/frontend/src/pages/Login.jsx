...
// Fond + halos partagé entre login et état "submitting"
function LoginBg({ haloEls }) {
  return (
    <>
      <div className="ll-bg" />
      ...
      <div className="ll-halos">
        {HALOS.map(({ cls, light }, i) => (
          <div key={cls} className={`ll-hw ${cls}`}
            ref={el => { if (haloEls) haloEls.current[i] = { el, light }; }}>
...
export default function Login() {
  ...
  const [submitting, setSubmitting] = useState(false); // true dès le clic, avant réponse API
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setError('');
    // ── Bascule immédiate vers l'état "loader" avant même l'appel API ─────
    // Le DOM rendu est identique à LoginLoader(fromLogin) → zéro jank au swap
    setSubmitting(true);
    try {
      await login(email, password);
      // currentUser set → App.jsx (useLayoutEffect) monte LoginLoader, Login unmount
      // Le visuel est déjà identique, la transition est invisible
    } catch (err) {
      setError(err.message);
      setSubmitting(false); // retour au formulaire avec l'erreur
    }
  };

  // ── État "submitting" : DOM identique à LoginLoader(fromLogin) ───────────
  // Même structure, mêmes classes, mêmes positions → swap invisible
  if (submitting) {
    return (
      <div className="ll-root">
        <LoginBg haloEls={haloEls} />
        <div className="ll-content">
          <img
            src="/logoCDC.png"
            alt="Cité des Chances"
            className="ll-logo"
            style={{ opacity: 1, animation: 'll-float-logo 4s ease-in-out infinite' }}
            onError={e => { e.currentTarget.style.display = 'none'; }}
          />
          {/* Barre de progression indéterminée pendant l'appel API */}
          <div className="ll-progress-track" style={{ opacity: 1 }}>
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          <img
            src="/logoCDC.png"
            alt="Cité des Chances"
            style={{ width: 72, height: 72, objectFit: 'contain', display: 'block', margin: '0 auto 16px', filter: 'drop-shadow(0 0 20px rgba(255,255,255,0.5))', animation: 'll-reveal-logo 0.8s ease both, ll-float-logo 4s ease-in-out infinite 1s' }}
            onError={e => { e.currentTarget.style.display = 'none'; }}
          />
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 800, color: '#fff', letterSpacing: '0.04em', textTransform: 'uppercase', marginBottom: 5, textShadow: '0 2px 12px rgba(255,255,255,0.15)' }}>
            Cité des Chances
          </div>
        ...
        {/* Carte */}
        <div style={{ width: '100%', background: 'rgba(255,255,255,0.05)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 24, padding: '36px 40px', boxShadow: '0 40px 80px rgba(0,0,0,0.5)', animation: 'll-slide-up 0.7s cubic-bezier(.16,1,.3,1) both' }}>
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            <div>
              ...
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} required autoFocus placeholder="prenom@citedeschances.com"
                style={{ width: '100%', boxSizing: 'border-box', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 12, padding: '13px 16px', fontSize: 14, color: '#fff', outline: 'none', transition: 'border-color .2s,background .2s,box-shadow .2s' }}
            ...
            <div>
              ...
              <div style={{ position: 'relative' }}>
                <input type={showPwd ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} required placeholder="••••••••"
                  style={{ width: '100%', boxSizing: 'border-box', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 12, padding: '13px 48px 13px 16px', fontSize: 14, color: '#fff', outline: 'none', transition: 'border-color .2s,background .2s,box-shadow .2s' }}
                  onFocus={e => { e.target.style.borderColor = 'rgba(59,130,246,0.7)'; e.target.style.background = 'rgba(255,255,255,0.09)'; e.target.style.boxShadow = '0 0 0 3px rgba(59,130,246,0.15)'; }}
                  onBlur={e => { e.target.style.borderColor = 'rgba(255,255,255,0.12)'; e.target.style.background = 'rgba(255,255,255,0.06)'; e.target.style.boxShadow = 'none'; }}
                />
            ...
            <button type="submit"
              style={{ background: 'linear-gradient(135deg,#1a56db 0%,#1e3a8a 100%)', color: '#fff', border: 'none', borderRadius: 12, padding: '14px 24px', fontSize: 14, fontWeight: 700, cursor: 'pointer', transition: 'all .2s', boxShadow: '0 4px 20px rgba(26,86,219,0.45)', marginTop: 4, letterSpacing: '0.02em' }}
